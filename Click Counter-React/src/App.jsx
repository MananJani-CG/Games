...
function App() {
  ...
  const endGame = () => {
    ...
    if (finalScore > highScore) {
      localStorage.setItem('clickGameHighScore', finalScore.toString());
      setHighScore(finalScore);
      setStatusMessage(`🎉 New High Score: ${finalScore}! Amazing!`);
      setShowVideo(true);
      if (videoRef.current) {
      ...
      setStatusMessage(`Game Over! Your score: ${finalScore}`);
    }
  };
  ...
  return (
    <div className="font-['Segoe_UI',Tahoma,Geneva,Verdana,sans-serif] bg-gradient-to-br from-[#667eea] to-[#764ba2] min-h-screen flex justify-center items-center p-5">
      <div className="bg-white rounded-[20px] p-10 shadow-[0_20px_60px_rgba(0,0,0,0.3)] max-w-[600px] w-full text-center z-10">
          ...
          <div className="flex gap-5 mb-[30px]">
              <div className="flex-1 bg-gradient-to-br from-[#667eea] to-[#764ba2] rounded-[15px] p-5 text-white">
                  ...
                  <div className="text-[3em] font-bold drop-shadow-[2px_2px_4px_rgba(0,0,0,0.2)]">{currentScore}</div>
              </div>
              <div className="flex-1 bg-gradient-to-br from-[#667eea] to-[#764ba2] rounded-[15px] p-5 text-white">
